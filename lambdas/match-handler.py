...
Filtros obligatorios (excluyen el par si no se cumplen):
  ...
  2. implant.tipo_implante == request.tejido_solicitado  (sin distinción de mayúsculas)
  3. implant.alto  >= request.alto_requerido  - 5  (rango máximo de déficit: 5 mm)
  4. implant.ancho >= request.ancho_requerido - 5  (ídem)
  5. Si request.sexo_importante: donor.sexo_biologico == patient.sexo_biologico
  6. No existe ya un match activo para (request_id, implant_id)

Scoring — cada componente vale entre 0 y 100, ponderados así:

  compatibility_alto  →  35 %
  compatibility_ancho →  35 %
  frescura del tejido →  20 %   (días restantes / vida útil total × 100)
...
def run_engine():
    ...
    for req in requests:
        ...
        for imp in implants:
            ...
            # — Filtro 1: tipo de tejido
            if imp["tipo_implante"].strip().lower() != req["tejido_solicitado"].strip().lower():
                continue
            ...
            if alto_diff < -5 or ancho_diff < -5:
                continue
                    ...
                    cur.execute(
                        ...
                        INSERT INTO match
                            (request_id, implant_id, compatibility_score,
                             compatibility_alto, compatibility_ancho, status)
                        VALUES (%s, %s, %s, %s, %s, 'detectado')
                        """,
                        (
                            req["id"],
                            c["implant_id"],
